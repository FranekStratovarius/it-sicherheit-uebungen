...
bitBlock *pad(bitBlock *block) {
    ...
    result->usedBits = block->usedBits;
	return result;
}
...
uint32 *sha1Faster(bitBlock *message) {
    uint32* result = (uint32*)calloc(5, sizeof(uint32));
    uint32 a0 = 0x67452301;
    uint32 b0 = 0xEFCDAB89;
    uint32 c0 = 0x98BADCFE;
    ...
    while (globalWordCounter < message->wordCount) {
        ...
        // STAGE 2 -------------
        for (roundCounter = 20; roundCounter < 40; roundCounter++) {
            if (roundCounter >= 16) {
                z = msf[(roundCounter - 16) % 16] ^
                    msf[(roundCounter - 14) % 16] ^
                    msf[(roundCounter - 8) % 16] ^
        ...
        // STAGE 3 -------------
        for (roundCounter = 40; roundCounter < 60; roundCounter++) {
            if (roundCounter >= 16) {
                z = msf[(roundCounter - 16) % 16] ^
                    msf[(roundCounter - 14) % 16] ^
                    msf[(roundCounter - 8) % 16] ^
        ...
        // STAGE 4 -------------
        for (roundCounter = 60; roundCounter < 80; roundCounter++) {
            if (roundCounter >= 16) {
                z = msf[(roundCounter - 16) % 16] ^
                    msf[(roundCounter - 14) % 16] ^
                    msf[(roundCounter - 8) % 16] ^
...
char *fromBitBlock(bitBlock * block) {
    uint32 charCount = block->usedBits / 8;
    uint32 i;
    uint32 charMasks[] = { 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF };
    char *word = (char*)calloc(charCount + 1, sizeof(char));
    for (i = 0; i < charCount; i++) {
        word[i] = (char)((block->data[i / 4] & charMasks[i % 4]) >> ((3 - (i % 4)) * 8));
    }
    word[i] = 0;
    return word;
}
